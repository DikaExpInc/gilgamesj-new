    ...
    try {
      ...
      const socialMedia = new SocialMedia({
        ...
        profile: `${profileImageFile.filename}.${profileImageFile.originalname
          .split(".")
          .pop()}`,
        post_image: `${postImageFile.filename}.${postImageFile.originalname
          .split(".")
          .pop()}`,
      });
    ...
    try {
      ...
      if (req.files) {
        if (req.files["profile"]) {
          ...
          profileImage = `${profileFile.filename}.${profileFile.originalname
            .split(".")
            .pop()}`;
        }
        ...
        if (req.files["post_image"]) {
          ...
          fs.renameSync(postImageFile.path, postImagePath);
          postImage = `${postImageFile.filename}.${postImageFile.originalname
            .split(".")
            .pop()}`;
        }
      }
