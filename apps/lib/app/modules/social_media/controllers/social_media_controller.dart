...
class SocialMediaController extends GetxController {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> streamSocialMedia() async* {
    ...
    List<String>? socialMediasIds =
        stageData['data_game']["social_media"]?.cast<String>();
    if (socialMediasIds == null || socialMediasIds.isEmpty) {
      return;
    }

    yield* firestore
        .collection("social_medias")
        .where(FieldPath.documentId, whereIn: socialMediasIds)
        .snapshots();
  }
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> streamTask() {
    var taskQuery = firestore
        ...
        .where('type', isEqualTo: 'social media')
        .snapshots();
