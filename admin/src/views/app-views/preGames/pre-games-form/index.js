...
const PreGamesForm = (props) => {
    ...
    if (mode === EDIT) {
      ...
      preGameService.getPreGame(id).then((querySnapshot) => {
        ...
        form.setFieldsValue({
          ...
          mode: querySnapshot.data.mode,
        })
        setImage(`${BASE_URL}${querySnapshot.data.icon}`)
        setLoadingData(false)
        if (querySnapshot.data.mode === 'chat') {
          selectValues.mode_selected = 'chat'
          form.setFieldsValue({
            chat_id: querySnapshot.data.chat_id,
          })
        }
        if (querySnapshot.data.mode === 'camera') {
          selectValues.mode_selected = 'camera'
          form.setFieldsValue({
            camera_id: querySnapshot.data.camera_id,
          })
        }
      })
    } else {
  ...
  }, [form, mode, param, props, selectValues])
